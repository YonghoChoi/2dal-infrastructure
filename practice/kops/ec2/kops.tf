...
resource "aws_instance" "kops" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "kubectl version",
      # "export KOPS_STATE_STORE=${var.k8s_state_store}",
      # "export KOPS_CLUSTER_NAME=${var.k8s_cluster_name}",
      # "export KUBECONFIG=~/.kube/${var.k8s_cluster_name}",
      "cat /dev/zero | ssh-keygen -q -N ''",
      "kops create secret --name ${var.k8s_cluster_name} --state ${var.k8s_state_store} sshpublickey admin -i ~/.ssh/id_rsa.pub",
      "kops delete cluster --name ${var.k8s_cluster_name} --state ${var.k8s_state_store}",
